%% %%% Test case definiton %%%%%%%%%%%%%%%%%%%%%%%% %%
    ...
    path = 'Stor kurva';  %'Rak bana'  'Stor kurva' 'Skarp kurva' 'Sinusvåg' 
...
%----------------Rak bana----------------
    ...
    for i=1:n
        X_waypoints(i)=-(i/100);
        X_waypointsUp(i)=-(i/100);
        X_waypointsLow(i)=-(i/100);
    end
...
case 'Stor kurva'
    ...
    Y_waypointsUp(1:200) = roadWidth;
    Y_waypointsLow(1:200) = -roadWidth;

    R=5; %Tuning parameter
    angles = zeros(1,800);
    angles(1)=-pi/2;
    for i=2:801
        angles(i)= angles(i-1) - ((pi/2)/800);
        X_waypoints(199+i)=R*cos(angles(i))-2;
        Y_waypoints(199+i)=R*sin(angles(i))+R;
        X_waypointsUp(199+i)=(R-roadWidth)*cos(angles(i))-2;
        Y_waypointsUp(199+i)=(R-roadWidth)*sin(angles(i))+R;
        X_waypointsLow(199+i)=(R+roadWidth)*cos(angles(i))-2;
        Y_waypointsLow(199+i)=(R+roadWidth)*sin(angles(i))+R;
    end

    for i=1:200
        Y_waypoints(i+1000)=(i/100)+R;
        Y_waypointsUp(i+1000)=(i/100)+R;
        Y_waypointsLow(i+1000)=(i/100)+R;
    end
    X_waypoints(1001:1200) = -(2+R);
    X_waypointsUp(1001:1200) = -(2+R-roadWidth);
    X_waypointsLow(1001:1200) = -(2+R+roadWidth);
    plot(X_waypointsLow,Y_waypointsLow)
%----------------Skarp kurva----------------
case 'Skarp kurva'
